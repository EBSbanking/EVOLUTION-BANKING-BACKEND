...
// In constants/roleMapping.js - Fix roleHasPermission

// constants/roleMapping.js - Update roleHasPermission

export async function roleHasPermission(roleId, permission) {
  try {
    // If no roleId provided, return false
    if (!roleId) {
      console.warn('⚠️ No roleId provided to roleHasPermission');
      return false;
    }
    ...
    try {
      ...
      const dbPermissions = await Permissions.findOne({ 
        where: { BU_ROLE_ID: roleId } 
      });
      ...
      if (dbPermissions) {
        ...
        // Check DEBIT_CARD_ACCESS_LEVEL
        if (dbPermissions.DEBIT_CARD_ACCESS_LEVEL && Array.isArray(dbPermissions.DEBIT_CARD_ACCESS_LEVEL)) {
          const hasPermission = dbPermissions.DEBIT_CARD_ACCESS_LEVEL.some(p => 
            p && p.trim().toUpperCase() === checkPermission
          );
          if (hasPermission) {
        ...
        for (const field of permissionFields) {
          if (dbPermissions[field] && Array.isArray(dbPermissions[field])) {
            const hasPermission = dbPermissions[field].some(p => 
              p && p.trim().toUpperCase() === checkPermission
            );
            if (hasPermission) {
              console.log(`✅ Permission found in ${field}`);
              return true;
            }
          }
        }
      ...
      console.warn('⚠️ Database permission check failed:', dbError.message);
    }
    ...
    // Check all permission groups in ROLE_MAPPING
    if (role.permissions) {
      for (const [group, permissions] of Object.entries(role.permissions)) {
        if (Array.isArray(permissions)) {
          const hasPermission = permissions.some(p => 
            p && p.trim().toUpperCase() === checkPermission
          );
          if (hasPermission) {
            console.log(`✅ Permission found in ${group} in ROLE_MAPPING`);
            return true;
          }
        }
      }

...
const creditApplicationSchema = new mongoose.Schema({
  ...
  PRODUCT: { type: String },
  ACCT_ID: { type: String, required: false }, // Changed to String for consistency
  ACCT_NO: { type: String, required: false }, // Changed to String for consistency
  APPL_DT: { type: Date, default: Date.now },
  APPL_ID: { type: String, unique: true },
  PROD_ID: { type: String, required: true },
  APPROVAL_DT: { type: Date, required: false },
  APPROVED_CRNCY_ID: { type: String },
  APPROVED_CR_REQD_DT: { type: Date },
  APPROVED_EXPIRY_DT: { type: Date },
  APPROVED_LIMIT_AMT: { type: Number, required: false },
  ...
  Credit_Type: { type: String, required: false },
  ...
  PRIME_LIMIT_AMT: { type: String, required: false },
  Product_Combination: { type: String },
  PROD_COMB_OPTION: { type: String },
  Purpose_of_Credit: { type: String, required: false },
...
creditApplicationSchema.pre('save', async function (next) {
  try {
    ...
    if (this.STATUS === 'PENDING') {
      this.Credit_Type = this.Credit_Type || 'LOAN';
      this.Purpose_of_Credit = this.Purpose_of_Credit || this.PRODUCT_TYPE || this.loan_purpose || 'GENERAL LOAN';
      this.PRIME_LIMIT_AMT = this.PRIME_LIMIT_AMT || this.DISBURSEMENT_LIMIT?.toString() || '1000000';
      this.APPROVAL_DT = this.APPROVAL_DT || null;
      this.APPROVED_LIMIT_AMT = this.APPROVED_LIMIT_AMT || null;
    }

    // Set CUST_NM if not provided
    if (!this.CUST_NM) {
      this.CUST_NM = this.ACCT_NM || 'Unknown Borrower';
    }

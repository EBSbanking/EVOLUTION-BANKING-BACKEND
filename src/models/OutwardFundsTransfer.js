...
class OutwardFundsTransfer extends Model {
  ...
  get charges() {
    return {
      ...
      receivingBank: parseFloat(this.recievingBankChrg) || 0,
  ...
  calculateNetAmount() {
    ...
    const receivingCharges = parseFloat(this.recievingBankChrg) || 0;
  ...
  getSummary() {
    return {
      transferId: this.outwdFundsXferId,
  ...
  static mapWebhookData(data) {
    // Support both camelCase and snake_case input for backward compatibility
    return {
      ...
      recievingBankChrg: data.recievingBankChrg || data.RECIEVING_BANK_CHRG || 0,
...
OutwardFundsTransfer.init({
  ...
  recievingBankChrg: { type: DataTypes.DECIMAL(15, 2), allowNull: true, defaultValue: 0.00 },
  ...
  hooks: {
    beforeCreate: (transfer) => {
      ...
      if (!transfer.totalChrg) {
        ...
        const receiving = parseFloat(transfer.recievingBankChrg) || 0;

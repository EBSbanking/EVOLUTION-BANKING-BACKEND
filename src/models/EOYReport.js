...
class EOYReport extends Model {
  ...
  /**
   * Get reports by fiscal year
   */
  static async findByFiscalYear(fiscalYear, options = {}) {
  ...
  /**
   * Get latest report for a fiscal year
   */
  static async getLatestReport(fiscalYear, organizationCode, branchCode) {
    return await this.findOne({
      where: {
   ...
   */
  static async getSummaryStats(organizationCode, branchCode, fiscalYear = null) {
    const whereClause = {
      organization_code: organizationCode,
      branch_code: branchCode,
      status: 'COMPLETED'
    ...
    const stats = await this.findAll({
      ...
      attributes: [
        ...
        [sequelize.fn('SUM', sequelize.fn('JSON_EXTRACT', sequelize.col('report_data'), '$.summary.netProfit')), 'totalNetProfit'],
        [sequelize.fn('SUM', sequelize.fn('JSON_EXTRACT', sequelize.col('report_data'), '$.summary.revenueTotal')), 'totalRevenue'],
        [sequelize.fn('SUM', sequelize.fn('JSON_EXTRACT', sequelize.col('report_data'), '$.summary.expenseTotal')), 'totalExpenses']
      ],
      group: ['fiscal_year'],
      raw: true
    });
  ...
  /**
   * Get journal entries from report
   */
  getJournalEntries() {
    if (!this.report_data) return [];
...
EOYReport.init(
  {
    ...
    report_id: {
      ...
      allowNull: false,
      unique: true,
      comment: 'Unique report identifier (EOY-REPORT-YYYY-{timestamp})'
    },
    fiscal_year: {
      type: DataTypes.INTEGER,
      allowNull: false,
      comment: 'Fiscal year the report belongs to'
    },
    report_data: {
      type: DataTypes.JSON,
      allowNull: false,
  ...
  },
  {
    ...
    indexes: [
      ...
      { 
        name: 'idx_org_branch_fiscal', 
        fields: ['organization_code', 'branch_code', 'fiscal_year'] 
      }
    ],
    hooks: {
      beforeCreate: (report) => {
        if (!report.report_id) {
          ...
          report.report_id = `EOY-REPORT-${report.fiscal_year}-${timestamp}`;
        }
...
// Associations
EOYReport.associate = (models) => {
  // Belongs to Organization (if models.Organization exists)
  if (models.Organization) {
    EOYReport.belongsTo(models.Organization, {
  ...
  // Belongs to Branch (if models.Branch exists)
  if (models.Branch) {
    EOYReport.belongsTo(models.Branch, {
  ...
  // Belongs to User (if models.User exists)
  if (models.User) {
    EOYReport.belongsTo(models.User, {

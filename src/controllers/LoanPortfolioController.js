// src/controllers/LoanPortfolioController.js – Fixed with proper interest accrual from repayment schedules
...
const LoanPortfolioController = {
  ...
  createPortfolioRecord: async (req, res) => {
    ...
    try {
      ...
      if (!BRANCH_ID || !PROD_ID || !MONTH || !YEAR) {
        throw new Error('BRANCH_ID, PROD_ID, MONTH, YEAR required');
      }

      const existing = await LoanPortfolio.findOne({ 
      ...
      if (!product) {
        throw new Error(`Product not found: ${PROD_ID}`);
      }
      ...
      // ✅ Calculate all metrics including interest
  ...
  updatePortfolioRecord: async (req, res) => {
    ...
    try {
      ...
      if (!portfolio) {
        throw new Error('Portfolio record not found');
      }
  ...
  getPortfolioById: async (req, res) => {
    try {
      const { id } = req.params;
      const record = await LoanPortfolio.findByPk(id, { 
        include: [{ 
          model: LoanProduct, 
          as: 'product', 
      ...
      data.COLLECTION_EFFICIENCY = data.TOTAL_REPAYMENTS > 0 
        ? (data.TOTAL_RECOVERED / data.TOTAL_REPAYMENTS) * 100 
        : 0;
      data.DEFAULT_RATE = data.NUMBER_OF_LOANS > 0 
        ? (data.TOTAL_DEFAULTS / data.NUMBER_OF_LOANS) * 100 
        : 0;
  ...
  getPortfolioByPeriod: async (req, res) => {
    try {
      ...
      const records = await LoanPortfolio.findAll({ 
        where, 
        include: [{ 
          model: LoanProduct, 
          as: 'product', 
  ...
  getPortfolioByBranch: async (req, res) => {
    try {
      ...
      const records = await LoanPortfolio.findAll({ 
        where, 
        include: [{ model: LoanProduct, as: 'product' }] 
      });
      ...
      records.forEach(r => {
        ...
        grouped[period].TOTAL_INTEREST_ACCRUED += d.TOTAL_INTEREST_ACCRUED || 0;
    ...
    try {
      ...
      records.forEach(r => {
        ...
        if (!branch) {
          ...
          branch.TOTAL_DISBURSED += d.TOTAL_DISBURSED;
          branch.OUTSTANDING_PRINCIPAL += d.OUTSTANDING_PRINCIPAL;
          branch.TOTAL_INTEREST_ACCRUED += d.TOTAL_INTEREST_ACCRUED;
        }
      });
  ...
  searchPortfolio: async (req, res) => {
    try {
      const { 
        BRANCH_ID, PROD_ID, PRODUCT_TYPE, YEAR, MONTH, STATUS, 
      ...
      const { count, rows } = await LoanPortfolio.findAndCountAll({
        where,
        offset: (page - 1) * limit,
  ...
  getPortfolioSummary: async (req, res) => {
    try {
      ...
      const nplRatio = summary.OUTSTANDING_PRINCIPAL 
        ? (summary.PORTFOLIO_AT_RISK / summary.OUTSTANDING_PRINCIPAL) * 100 
        : 0;
      
      // ✅ Calculate interest coverage ratio
      const interestCoverageRatio = summary.TOTAL_INTEREST_ACCRUED > 0 
        ? (summary.TOTAL_INTEREST_RECEIVED / summary.TOTAL_INTEREST_ACCRUED) * 100 
        : 0;
  ...
  getPortfolioHealth: async (req, res) => {
    try {
      ...
      records.forEach(r => {
        ...
        totalInterestAccrued += parseFloat(r.TOTAL_INTEREST_ACCRUED) || 0;
        totalInterestReceived += parseFloat(r.TOTAL_INTEREST_RECEIVED) || 0;
      });
      
      const nplRatio = totalOutstanding ? (totalPar / totalOutstanding) * 100 : 0;
      const interestCollectionRate = totalInterestAccrued > 0 
        ? (totalInterestReceived / totalInterestAccrued) * 100 
        : 0;
      
      const riskCategory = nplRatio < 5 ? 'LOW' 
        : nplRatio < 15 ? 'MODERATE' 
        : nplRatio < 30 ? 'HIGH' 
        : 'CRITICAL';
      ...
      res.json({
        ...
        data: {
          metrics: {
            ...
            INTEREST_COLLECTION_RATE: interestCollectionRate
          },
          ratios: {
            NPL_RATIO: nplRatio,
            INTEREST_COLLECTION_RATE: interestCollectionRate
  ...
  generatePortfolioForPeriod: async (req, res) => {
    ...
    try {
      ...
      if (existing.length) {
        throw new Error('Portfolio records already exist for this period');
      }
      ...
      res.status(201).json({
        ...
        data: generated.map(r => r.toJSON()),
        message: `Generated ${generated.length} portfolio records`
      });
    } catch (error) {
      await transaction.rollback();
  ...
  deletePortfolioRecord: async (req, res) => {
    ...
    try {
      ...
      if (record.STATUS === 'ACTIVE') {
        throw new Error('Cannot delete active records – archive first');
      }
      ...
      await transaction.rollback();
      console.error('deletePortfolioRecord error:', error);
      res.status(500).json({ 
        success: false, 
        message: error.message 
      });
  ...
  archivePortfolioRecord: async (req, res) => {
    ...
    try {
      ...
      await transaction.rollback();
      console.error('archivePortfolioRecord error:', error);
      res.status(500).json({ 
        success: false, 
        message: error.message 
      });
...
/**
 * Calculate portfolio metrics including interest accrued from repayment schedules
 */
async function calculatePortfolioMetrics(portfolioData, transaction) {
  ...
  // ✅ Get all loans for this branch/product during the period
  ...
  // ✅ Get repayment schedules to calculate interest accrued
  let totalInterestAccrued = 0;
  ...
  if (loanIds.length) {
    ...
    // ✅ Calculate total interest from all schedules
    for (const schedule of schedules) {
      // Use the total_interest from the repayment_schedule
      totalInterestAccrued += parseFloat(schedule.total_interest || 0);
    }
    ...
    for (const r of repayments) {
      ...
      totalInterestReceived += parseFloat(r.interest_amount || 0);
      totalFees += parseFloat(r.penalty_amount || 0);
    }
  }

  // ✅ Calculate derived metrics
  const nplRatio = outstandingPrincipal > 0 
    ? (portfolioAtRisk / outstandingPrincipal) * 100 
    : 0;
  
  const yieldRate = totalPrincipal > 0 
    ? (totalInterestReceived / totalPrincipal) * 100 * 12 
    : 0;
  ...
  // ✅ Update portfolio data with calculated metrics
  ...
  // ✅ CRITICAL: Set the interest accrued from repayment schedules
  ...
  // ✅ Provision amount based on portfolio at risk
  portfolioData.PROVISION_AMOUNT = portfolioAtRisk * 0.1; // 10% provision
  
  portfolioData.NPL_RATIO = nplRatio;
  portfolioData.YIELD_RATE = yieldRate;
  portfolioData.AVERAGE_LOAN_SIZE = averageLoanSize;
  
  // ✅ Additional metrics
  portfolioData.NET_INTEREST_MARGIN = totalPrincipal > 0 
    ? ((totalInterestReceived - 0) / totalPrincipal) * 100 
    : 0;
  
  portfolioData.COST_OF_FUNDS = 0; // This should come from another source
}

/**
 * Get distinct branches from loan accounts
 */
...
function convertToCSV(records) {
  ...
  const rows = records.map(r => {
    ...
    const npl = r.OUTSTANDING_PRINCIPAL 
      ? (r.PORTFOLIO_AT_RISK / r.OUTSTANDING_PRINCIPAL) * 100 
      : 0;
    ...
    return [
      ...
      r.TOTAL_INTEREST_ACCRUED || 0,
      r.TOTAL_INTEREST_RECEIVED || 0,
      r.NUMBER_OF_LOANS || 0,
      npl.toFixed(2),
      r.STATUS
    ];

...
const createTimeService = () => {
  ...
  const getBusinessDate = async (context = {}) => {
    ...
    try {
      ...
      if (systemDate?.current_business_date) {
        // If time is frozen, use frozen time with the business date
        if (isFrozen && frozenTime) {
          const frozenDate = new Date(frozenTime);
          frozenDate.setHours(0, 0, 0, 0);
          return frozenDate;
        }
        return new Date(new Date(systemDate.current_business_date).getTime() + WAT_OFFSET);
      }

      logger.warn('No system date found, using server time');
